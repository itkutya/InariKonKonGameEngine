...
namespace ikk
{
    ...
    class Swapchain final : public VkType<VkSwapchainKHR>
    {
    public:
        Swapchain(const std::uint32_t width, const std::uint32_t height, Surface& surface, PhysicalDevice& physicalDevice, LogicalDevice& logicalDevice, const std::size_t maxFramesInFlight) noexcept;
        ...
        void create(const std::uint32_t width, const std::uint32_t height, PhysicalDevice& physicalDevice) noexcept;
        void destroy() noexcept;
        
        [[nodiscard]] const VkResult acquireNextImage(std::uint32_t* imageIndex, const std::uint64_t timeout, const std::uint32_t frameIndex) noexcept;
